...
$pageUrl = (string) ($argv[2] ?? '');
$textHint = (string) ($argv[3] ?? '');

if ($pageUrl === '') {
    fwrite(STDERR, "Usage: php bin/debug-facebook-feed.php <session_id> <page_url> [text_hint]\n");
    exit(1);
}
...
$screenshotPath = $screenshotDir . '/debug-fb-feed-' . time() . '.png';
...
$proc = proc_open(
    [NodeProcess::nodeBinary(), BASE_DIR . '/automation/var/cache/debug-fb-permalink.mjs'],
    [
        0 => ['pipe', 'r'],
        1 => ['pipe', 'w'],
        2 => ['pipe', 'w'],
