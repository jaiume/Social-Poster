...
if ($postId <= 0) {
    fwrite(STDERR, "Usage: php bin/test-facebook-post.php <post_id> [--dry-run]\n");
    exit(1);
}
...
$slot = $postingDao->findForProfilePlatform((int) $post['product_profile_id'], 'facebook');
if ($slot === null) {
...
$primaryPageBrand = preg_replace('/\s+Facebook$/i', '', (string) ($account['display_name'] ?? ''));
$payload = [
    'text' => $content,
...
echo ($dryRun ? 'Dry-run' : 'Live') . " Facebook post for post {$postId}\n";
...
$result = $poster->publishFacebookPrimary($sessionId, $payload);
...
echo "Facebook post " . ($dryRun ? 'dry-run' : 'publish') . " OK.\n";
