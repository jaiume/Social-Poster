...
if ($postId <= 0 || $primaryPostUrl === '') {
    fwrite(STDERR, "Usage: php bin/test-linkedin-repost.php <post_id> <primary_post_url>\n");
    exit(1);
}
...
$postingSlot = $postingDao->findForProfilePlatform($profileId, 'linkedin');
if ($postingSlot === null) {
...
if ($repostRow === null) {
    fwrite(STDERR, "Post {$postId} needs a LinkedIn repost account.\n");
    exit(1);
}
...
$personalUrl = SessionAccountUrls::personalContextUrl('linkedin');
$primaryPageBrand = preg_replace('/\s+Linkedin$/i', '', (string) ($postingAccount['display_name'] ?? ''));
$personalProfileName = SessionAccountUrls::memberNameFromDisplayName('linkedin', (string) ($repostAccount['display_name'] ?? ''));
$payload = [
    'text' => $content,
...
if ($personalProfileName !== null && $personalProfileName !== '') {
    $payload['personalProfileName'] = $personalProfileName;
}
$payload['personalProfileUrl'] = 'https://www.linkedin.com/in/jamie-mclachlan';

echo "LinkedIn repost for post {$postId}\n";
...
$result = $poster->publishLinkedInRepost($sessionId, $payload);
